...
- Automatically owning arbitrary foreign memory.
...
local elem = ptr:deref() -- get a view of the first element
```
...
`ffi.open` accepts either a plain library name (e.g. `"c"`, `"m"`) or a full path. `OpenOptions` maps to platform dlopen flags: `global` corresponds to `RTLD_GLOBAL` and `now` to `RTLD_NOW`.
...
`c.void` is the void type. It cannot be instantiated.
...
5. Memory helpers (`c.string`)
6. Conversions (`c.cast`)
7. Hardening, docs, and conformance tests
